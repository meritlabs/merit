...
*Setup instructions for a Gitian build of Merit Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Merit
...
- [Building Merit Core](#building-merit-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-merit-host/README.md).
...
Clone the git repositories for merit and Gitian.
...
in the merit repository under 'Fetch and create inputs' to install sources which require
...
To build Merit Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the merit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/merit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/meritlabs/merit
...
For example:
```bash
URL=https://github.com/laanwj/merit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit merit=${COMMIT} --url merit=${URL} ../merit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit merit=${COMMIT} --url merit=${URL} ../merit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit merit=${COMMIT} --url merit=${URL} ../merit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the merit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../merit/contrib/gitian-descriptors/*|sort|uniq )
...
MRTPATH=/some/root/path/merit
SIGPATH=/some/root/path/merit-detached-sigs

./bin/gbuild --url merit=${MRTPATH},signature=${SIGPATH} ../merit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/merit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/merit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/merit-osx-build.assert
```
...
[merit-core/gitian.sigs](https://github.com/bitcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
