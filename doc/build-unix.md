UNIX BUILD NOTES
====================
Some notes on how to build Merit Core in Unix.

(for OpenBSD specific instructions, see [build-openbsd.md](build-openbsd.md))

Note
---------------------
Always use absolute paths to configure and compile merit and the dependencies,
...
memory available when compiling Merit Core. On systems with less, gcc can be
...
**For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~merit/+archive/merit).
...
See the section "Disable-wallet mode" to build Merit Core without wallet.
...
To build with Qt 5 (recommended) you need the following:
    ...
    sudo dnf install qt5-qttools-devel qt5-qtbase-devel protobuf-devel libarchive-devel
...
Notes
-----
The release is built with GCC and then "strip meritd" to strip the debug
...
# Configure Merit Core to use our own-built instance of BDB
cd $MERIT_ROOT
./autogen.sh
...
Security
--------
To help make your merit installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, merit should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, merit may be compiled in
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/merit/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard Merit Core distributions and independently built
...
with 4.8-built Merit Core is needed follow the steps under "Berkeley DB" above.
