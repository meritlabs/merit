...
8445 for mainnet, port 18445 for testnet, and port 18556 for regtest.
...
"txindex=1" command line / configuration option.
...
* mempoolminfee : (numeric) minimum feerate (MRT per KB) for tx to be accepted
...
Risks
-------------
Running a web browser on the same node with a REST enabled meritd can be a risk. Accessing prepared XSS websites could read out tx/block data of your node by placing links like `<script src="http://127.0.0.1:8332/rest/tx/1234567890.json">` which might break the nodes privacy.
