...
The RPM spec file provided here is for Merit-Core 0.12.0 and builds on CentOS
...
[Merit git repository](https://github.com/meritlabs/merit) but are not part of
...
Currently only `Source10` is used, for the example `merit.conf` file.
...
in the merit git repository. At present only an SVG version of the Merit
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the merit
...
needed when 0.12.1 is released, a proper fix is already in the Merit git
master branch.
...
idea to build Merit Core against a static version of that library compiled
...
The `merit.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `merit-server` package. At this point, most RPM based
...
The `merit-server` package should have SELinux support. How to properly do
...
Merit-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/merit/). He has been
packaging Merit for Fedora at least since 2012.
...
4. I renamed the `merit` package that contains the Qt GUI to `merit-core` as
that appears to be how the general population refers to it, in contrast to
`merit-xt` or `merit-classic`. I wanted to make sure the general population
