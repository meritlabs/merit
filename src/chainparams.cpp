...
CBlock CreateNewGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward)
{
...
CBlock CreateNewGenesisBlock(uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward)
{
    ...
    return CreateNewGenesisBlock(pszTimestamp, genesisOutputScript, nTime, nNonce, nBits, nVersion, genesisReward);
}
...
public:
    CTestNetParams() {
        ...
        //ours
        //genesis = CreateGenesisBlock(1503422445, 238117321, 0x1d00ffff, 1, 50 * COIN);
        ...
        std::cerr << "Show me the genesis!" << std::endl;
        std::cerr << genesis.ToString() << std::endl;
