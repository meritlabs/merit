    ...
    bool findPoW)
{
    ...
    if (findPoW) {
        ...
        while (nMaxTries > 0 && !cuckoo::FindProofOfWorkAdvanced(genesis.GetHash(), genesis.nBits, genesis.nEdgeBits, pow, params, -1)) {
            ++genesis.nNonce;
            --nMaxTries;
        }
...
void runEdgeBitsGenerator(Consensus::Params& consensus)
{
    ...
    for (const auto& edgeBits : bits) {
        ...
        while (nMaxTries > 0 && !found) {
            ...
            found = cuckoo::FindProofOfWorkAdvanced(genesis.GetHash(), genesis.nBits, genesis.nEdgeBits, pow, consensus, -1);
