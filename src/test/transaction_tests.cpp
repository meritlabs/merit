	    ...
	    for (unsigned int inpIdx = 0; inpIdx < inputs.size(); inpIdx++) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++)
            {
                ...
                BOOST_CHECK_MESSAGE(VerifyScript(tx.vin[i].scriptSig, mapprevOutScriptPubKeys[tx.vin[i].prevout],
                                                 witness, verify_flags, TransactionSignatureChecker(&tx, i, amount, 0, 0, txdata), &err),
                                    strTest);
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction) {
    ...
    for(uint32_t i = 0; i < mtx.vin.size(); i++) {
        ...
        CScriptCheck check(output.scriptPubKey, output.nValue, tx, i, 0, 0, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, false, &txdata);
...
BOOST_AUTO_TEST_CASE(test_witness)
{
    ...
    keystore.AddCScript(scriptPubkey1, CScriptID{scriptPubkey1});
    keystore.AddCScript(scriptPubkey2, CScriptID{scriptPubkey2});
    keystore.AddCScript(scriptPubkey1L, CScriptID{scriptPubkey1L});
    keystore.AddCScript(scriptPubkey2L, CScriptID{scriptPubkey2L});
    keystore.AddCScript(scriptMulti, CScriptID{scriptMulti});
    keystore.AddCScript(GetScriptForWitness(scriptPubkey1), CScriptID{GetScriptForWitness(scriptPubkey1)});
    keystore.AddCScript(GetScriptForWitness(scriptPubkey2), CScriptID{GetScriptForWitness(scriptPubkey2)});
    keystore.AddCScript(GetScriptForWitness(scriptPubkey1L), CScriptID{GetScriptForWitness(scriptPubkey1L)});
    keystore.AddCScript(GetScriptForWitness(scriptPubkey2L), CScriptID{GetScriptForWitness(scriptPubkey2L)});
    keystore.AddCScript(GetScriptForWitness(scriptMulti), CScriptID{GetScriptForWitness(scriptMulti)});
    keystore2.AddCScript(scriptMulti, CScriptID{scriptMulti});
    keystore2.AddCScript(GetScriptForWitness(scriptMulti), CScriptID{GetScriptForWitness(scriptMulti)});
    ...
    UpdateTransaction(input1, 0, CombineSignatures(output1->vout[0].scriptPubKey, MutableTransactionSignatureChecker(&input1, 0, output1->vout[0].nValue, 0, 0), DataFromTransaction(input1, 0), DataFromTransaction(input2, 0)));
    CheckWithFlag(output1, input1, STANDARD_SCRIPT_VERIFY_FLAGS, true);
