...
const std::string MeritGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MeritGUI::DEFAULT_WALLET = "~Default";
...
void MeritGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Merit address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and merit: URIs)"));
    ...
    exportWalletQRAction->setVisible(false);
#endif
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Merit addresses to prove you own them"));
    signMessageAction->setVisible(false);

    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Merit addresses"));
    ...
    openAction->setStatusTip(tr("Open a merit: URI or payment request"));
...
void MeritGUI::setClientModel(ClientModel *_clientModel)
{
...
void MeritGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MeritGUI::walletReferred()
{
    ...
    if(walletFrame) {
        walletFrame->UpdateOverviewPage();
    }
}
#endif // ENABLE_WALLET
...
void MeritGUI::setWalletActionsEnabled(bool enabled, bool isReferred)
{
    ...
    importWalletAction->setVisible(!isReferred);
...
void MeritGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MeritGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MeritGUI::showDebugWindowActivateConsole()
{
...
void MeritGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MeritGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MeritGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MeritGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MeritGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MeritGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void MeritGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Merit network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MeritGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MeritGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MeritGUI::updateHeadersSyncProgressLabel()
{
...
void MeritGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void MeritGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Merit"); // default title
...
void MeritGUI::changeEvent(QEvent *e)
{
...
void MeritGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MeritGUI::showEvent(QShowEvent *event)
{
...
void MeritGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MeritGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MeritGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MeritGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MeritGUI::setHDStatus(int hdEnabled)
{
...
void MeritGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MeritGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MeritGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MeritGUI::detectShutdown()
{
...
void MeritGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MeritGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MeritGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MeritGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void MeritGUI::subscribeToCoreSignals()
{
    ...
    uiInterface.ThreadSafeQuestion.connect(boost::bind(ThreadSafeMessageBox, this, boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_4));
}

void MeritGUI::unsubscribeFromCoreSignals()
{
    ...
    uiInterface.ThreadSafeQuestion.disconnect(boost::bind(ThreadSafeMessageBox, this, boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_4));
}

void MeritGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<MeritUnits::Unit> units = MeritUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MeritUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(MeritUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (MeritUnits::Unit u : MeritUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MeritUnits::name(u)), this);
...
{
    setText(MeritUnits::name(newUnits));
}
