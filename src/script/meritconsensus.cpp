...
inline int set_error(meritconsensus_error* ret, meritconsensus_error serror)
{
    if (ret)
        *ret = serror;
    return 0;
        ...
        meritconsensus_error* err)
{
    ...
    try {
        ...
        if (nIn >= tx.vin.size())
            return set_error(err, meritconsensus_ERR_TX_INDEX);
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen)
            return set_error(err, meritconsensus_ERR_TX_SIZE_MISMATCH);
