...
namespace shamir
{
    ...
    CPrivKey BigIntToKey(ff::BigInt val)
    {
        ...
        for(auto it = key.rbegin(); it != key.rend(); ++it) {
            ...
            *it = (unsigned char)bits;
            val >>= 8;
        }
        return key;
    }

    std::vector<ff::FieldPoint> ShardKey(const CPrivKey& key)
    {
        ...
        std::vector<ff::FieldPoint> points(N);
        std::transform(inputs.begin(), inputs.end(), points.begin(),
            [&coefs](const ff::Element input)
            {
                return ff::FieldPoint{input, ff::EvalPolynomial(input, coefs)};
            });
        return points;
    }
