 ...
 * between meritd, and merit-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), MERIT_CONF_FILENAME));
    if (mode == HMM_MERITD)
    {
#if HAVE_DECL_DAEMON
    ...
    strUsage += HelpMessageOpt("-maxrefmempool=<n>", strprintf(_("Keep the referrals memory pool below <n> megabytes (default: %u)"), DEFAULT_MAX_REFERRALS_MEMPOOL_SIZE));
    strUsage += HelpMessageOpt("-refmempoolexpiry=<n>", strprintf(_("Do not keep referrals in the mempool longer than <n> hours (default: %u)"), DEFAULT_REFERRALS_MEMPOOL_EXPIRY));
    if (showDebug) {
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), MERIT_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-validationsamplecount=<n>", _("Take 1 out of N samples for validation when initially downloading or reindexing the blockchain (\"0\" to run validation on each block)."));
#ifndef WIN32
...
/** Sanity checks
 *  Ensure that Merit is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (nReferralsMempoolSizeMax < 0)
        return InitError(strprintf(_("-maxrefmempool must be greater than zero")));
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Merit process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("* Using %.1fMiB for in-memory UTXO set (plus up to %.1fMiB of unused mempool space and %.1fMiB of unused referrals mempool space)\n",
        ...
        nReferralsMempoolSizeMax * (1.0 / 1024 / 1024));
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                // LoadBlockIndex will load tx index from the db, or set it if
